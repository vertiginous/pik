Feature: switch command
  ...
  Scenario Outline: Switching versions with switch command
    ...
    When I run "pik switch <patterns> -v" and check the environment
  ...
  Scenario Outline: Switching versions with pattern only
    Given I have added "<version>" to pik
    When I run "pik <patterns>" and check the environment
   ...
   Scenario Outline: Switching versions and gemsets
    Given I have added "<version>" to pik
    When I run "pik sw <patterns> -m albinochipmunk" and check the environment
    Then the path should point to it.
    And the "GEM_HOME" variable will include "%albinochipmunk".
